...
# Public: The following Webdriver code tells the kaikifs show an Item's sub-item
#         based on its ordered position.
#
# Parameters:
...
# Public: The following Webdriver code tells the kaikifs to click on a form item
#         that is within another element.
#
# Parameters:
...
When /^I click "([^"]*)" under (.*)$/ do |button, tab|
  ...
  when button =~ /inactive/
    kaiki.click_and_wait(:xpath, "//h2[contains(text(), '#{tab}')]/../following-sibling::*//input[contains(@title, 'inactive')]")
  end
end
