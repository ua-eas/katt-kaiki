...
When /^I click the "([^"]*)" button$/ do |item|
  ...
  if item == 'create_new'
    ...
    button = kaiki.find(:name, "methodToCall.performLookup.(!!org.kuali.kra.bo.KcPerson!!).(((personId:newPersonId))).((``)).((<>)).(([])).((**)).((^^)).((&&)).((//)).((~~)).(::::;;::::).anchor")
    button.click
  elsif item == 'non-employee search lookup'
    button = kaiki.find(:name, "methodToCall.performLookup.(!!org.kuali.kra.bo.NonOrganizationalRolodex!!).(((rolodexId:newRolodexId))).((``)).((<>)).(([])).((**)).((^^)).((&&)).((//)).((~~)).(::::;;::::).anchor")
    button.click
  elsif item == 'turn on validation'
    button = kaiki.find(:name, 'methodToCall.activate')
    button.click
  elsif item == 'add'
    kaiki.click_by_xpath("/html/body/form/table/tbody/tr/td[2]/div[2]/div/div[2]/table/tbody/tr[2]/td[8]/div/input")
  elsif item == 'document search'
    button = kaiki.find(:xpath,'/html/body/div[5]/div/div/a[3]')
    button.click
  elsif item == 'document link'
    button = kaiki.find(:xpath,'/html/body/form/table/tbody/tr/td[2]/table/tbody/tr/td/a')
    button.click
  else
